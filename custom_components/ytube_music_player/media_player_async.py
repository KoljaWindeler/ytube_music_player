...
class yTubeMusicComponent(MediaPlayerEntity):
	def __init__(self, hass, config, name_add):
		...
		self._interrupt_downloader = False
	...
	async def async_get_track(self, entity_id=None, old_state=None, new_state=None, retry=3):
		...
		#try:
			...
			if(self._proxy_url != "" and self._proxy_path != ""):
				...
				while(self._interrupt_downloader):
					asyncio.wait(0.5)
				self.hass.loop.create_task(self.async_downloader(stream_url,os.path.join(self._proxy_path, PROXY_FILENAME),_url))
	...
	async def async_downloader(self,download_url="",download_path="", local_url=""):
		self.log_me('debug', "HI this is the downloader")
		p1 = datetime.datetime.now()
		async with aiohttp.ClientSession(loop=self.hass.loop) as session:
			async with session.get(download_url) as response:
				with open(download_path, 'wb') as f_handle:
					playing=False
					i=0
					while True:
						chunk = await response.content.read(128*80*1024)
						i+=len(chunk)
						if not chunk:
							break
						if(self._interrupt_downloader):
							self.log_me('debug', "- Downloader interrupted")
							self._interrupt_downloader = False
							break
						self.log_me('debug', "downloaded "+str(i)+" B")
						f_handle.write(chunk)
						if(i>=500000 and not playing):
							playing = True
							data = {
								ATTR_MEDIA_CONTENT_ID: local_url,
								ATTR_MEDIA_CONTENT_TYPE: MEDIA_TYPE_MUSIC,
								ATTR_ENTITY_ID: self._remote_player,
								"extra": {
									"metadata": {
